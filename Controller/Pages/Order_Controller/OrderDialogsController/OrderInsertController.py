...
from View.Pages.Order_Page.Order_Dialogs.OrderInsertDialog import OrderInsertDialog


class OrderInsertController(QObject):
    refresh_requested = Signal()

    def __init__(self,theme,model):
        super(OrderInsertController,self).__init__()
        self.message = None
        self.is_dark = theme
        self.model = model
        self.view = None
    ...
    def setup_view(self):
        ...
        self.view = OrderInsertDialog(self.is_dark, names)
    ...
    def insert_requested(self):
        ...
        qdate = self.view.order_date.date()
        order_date = datetime(qdate.year(), qdate.month(), qdate.day())
        order_comment = self.none_if_empty(self.view.order_comment.text())
        ...
        result,success = self.model.insert(customer_name,customer_tel,customer_city,customer_address,order_date,order_comment)
