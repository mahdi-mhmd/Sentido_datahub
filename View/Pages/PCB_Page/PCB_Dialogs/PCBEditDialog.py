...
class PCBEditDialog(QDialog):
    edit_requested = Signal()
    close_requested = Signal()
    name_changed = Signal(str)

    def __init__(self, theme, name_list, current_data):
        super(PCBEditDialog, self).__init__()
        self.is_dark = theme
        self.setWindowTitle("Edit (PCB)")
    ...
    def setup_widgets(self, layout):
        ...
        for combobox, text, data in [(self.pcb_name, "Name", self.current_data[0]),
                                     (self.pcb_board_per_sheet, "Board/Sheet", str(self.current_data[1])),
                                     (self.pcb_color, "Color", self.current_data[2]),
                                     (self.pcb_finishing, "Finishing", self.current_data[3]),
                                     (self.pcb_thickness, "Thickness", self.current_data[4])]:
        ...
        for spinbox,text,data in ((self.pcb_sheet_qty, "Sheet_Qty", self.current_data[5]), (self.pcb_price, "Price", self.current_data[7])):
        ...
        self.pcb_comment.setText(self.current_data[8])
        v_layout = QVBoxLayout()
