...
class LOMInsertDialog(QDialog):
    insert_requested = Signal()
    close_requested = Signal()
    ec_type_changed = Signal(str)
    pcb_name_changed = Signal(str)

    def __init__(self, theme, type_list, name_list, feeder_list, nozzle_list):
        super(LOMInsertDialog, self).__init__()
        self.is_dark = theme
        self.setWindowTitle("Insert (LOM)")
    ...
    def setup_widgets(self, layout):
        ...
        for combobox, text in ((self.lom_feeder, "Feeder"), (self.lom_nozzle, "Nozzle")):
            combobox.setEditable(True)
            combobox.setFont(font)
            combobox.setMaxVisibleItems(5)
            v_layout = QVBoxLayout()
