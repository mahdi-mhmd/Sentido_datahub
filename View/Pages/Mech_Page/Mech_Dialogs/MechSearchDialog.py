...
class MechSearchDialog(QDialog):
    adv_search_requested = Signal()
    close_requested = Signal()
    type_changed = Signal(str)

    def __init__(self, theme, type_list, headers, info):
        super(MechSearchDialog, self).__init__()
        self.is_dark = theme
        self.setWindowTitle("ADV Search (Mechanical part)")
    ...
    def setup_comboboxes(self,layout):
        ...
        self.mech_type.setCurrentIndex(-1)

        for combobox, text in [(self.mech_type, "Type"), (self.mech_name, "Name"),(self.mech_color, "Color")]:
